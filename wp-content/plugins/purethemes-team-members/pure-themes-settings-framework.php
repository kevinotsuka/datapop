...
if( !class_exists('Pure_Member_Settings_Framework') ){
    /**
     * Framework class
     */
    class Pure_Member_Settings_Framework {
         ...
         */
        function __construct( $settings_file, $option_group = 'pure_plugin' )
        {
    	...
    	/**
         * Displays the "section_description" if speicified in $PMSF_settings
         *
    	...
    	function section_intro( $args )
    	{
        	...
        	if(!empty($PMSF_settings)){
        		foreach($PMSF_settings as $section){
    	...
    	/**
         * Processes $PMSF_settings and adds the sections and fields via the WordPress settings API
         */
    	function process_settings()
    	{
            global $PMSF_settings;
        	if(!empty($PMSF_settings)){
        	    usort($PMSF_settings, array(&$this, 'sort_array'));
        		foreach($PMSF_settings as $section){
    	...
    	/**
         * Usort callback. Sorts $PMSF_settings by "section_order"
         * 
    	...
    	function generate_setting( $args )
    	{
        	...
        	$defaults = apply_filters( 'PMSF_defaults', $defaults );
        	...
        	do_action('PMSF_before_field');
        	do_action('PMSF_before_field_'. $el_id);
    		switch( $type ){
    		    case 'text':
    		    ...
    		    case 'file':
                    $val = esc_attr($val);
    		        echo '<input type="text" name="'. $this->option_group .'_settings['. $el_id .']" id="'. $el_id .'" value="'. $val .'" class="regular-text '. $class .'" /> ';
                    echo '<input type="button" class="button PMSF-browse" id="'. $el_id .'_button" value="Browse" />';
                    echo '<script type="text/javascript">
                    jQuery(document).ready(function($){
    		...
    		do_action('PMSF_after_field');
        	do_action('PMSF_after_field_'. $el_id);
    	}
        ...
        function settings()
        {
            do_action('PMSF_before_settings');
            ?>
            <form action="options.php" method="post">
                <?php do_action('PMSF_before_settings_fields'); ?>
                <?php settings_fields( $this->option_group ); ?>
        		<?php do_settings_sections( $this->option_group ); ?>
        		<p class="submit"><input type="submit" class="button-primary" value="<?php _e( 'Save Changes' ); ?>" /></p>
			</form>
    		<?php
    		do_action('PMSF_after_settings');
        }
    
    }   
}

if( !function_exists('PMSF_get_option_group') ){
    /**
     ...
     */
    function PMSF_get_option_group( $settings_file ){
        $option_group = preg_replace("/[^a-z0-9]+/i", "", basename( $settings_file, '.php' ));
        return $option_group;
    }
}

if( !function_exists('PMSF_get_settings') ){
    /**
     ...
     */
    function PMSF_get_settings( $settings_file, $option_group = '' ){
...
if( !function_exists('PMSF_get_setting') ){
    /**
     ...
     */
    function PMSF_get_setting( $option_group, $section_id, $field_id ){
...
if( !function_exists('PMSF_delete_settings') ){
    /**
     ...
     * @param string optional "option_group" override
     */
    function PMSF_delete_settings( $settings_file, $option_group = '' ){
        $opt_group = preg_replace("/[^a-z0-9]+/i", "", basename( $settings_file, '.php' ));
        if( $option_group ) $opt_group = $option_group;
